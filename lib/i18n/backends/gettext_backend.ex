defmodule I18n.Backends.GettextBackend do
  alias I18n.Gettext.GettextParser
  alias I18n.Message
  require Logger

  defmacro __using__(opts \\ []) do
    otp_app = Keyword.fetch!(opts, :otp_app)
    priv = Keyword.get(opts, :priv, "priv/I18n")
  ...
  def log_message_not_extracted(message) do
    Logger.warn(
      "I18n - message not extracted (#{message.file}:#{message.line}): #{
  ...
  defp generate_clauses(fun_name, _messages = {original_messages, locales}, external_resources) do
    ...
    message_not_extracted_function_clauses =
      quote do
        ...
        def unquote(fun_name)(
          ...
          I18n.Backends.GettextBackend.log_message_not_extracted(message)
          ...
          I18n.Pseudolocalization.HtmlPseudolocalization.pseudolocalize(text)
        end
        ...
        def unquote(fun_name)(
          ...
          text = to_string(localized)
          # Use fully qualified module name in quoted expression
          I18n.Pseudolocalization.TextPseudolocalization.pseudolocalize(text)
        end
    ...
    # so that changing them triggers a recompilation of the I18n backend.
